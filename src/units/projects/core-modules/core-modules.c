...
} nxs_fw_ctl_u_projects_core_modules_using_distrib_t;
...
typedef struct
{
	...
	nxs_string_t					link_opts;
	nxs_string_t					includes;
	nxs_array_t					distribs;		/* type: nxs_fw_ctl_u_projects_core_modules_using_distrib_t */
	nxs_string_t					core_links;
} nxs_fw_ctl_u_projects_core_modules_using_t;
...
typedef struct
{
	...
	nxs_string_t					prompt;
	nxs_string_t					using;
} nxs_fw_ctl_u_projects_core_modules_select_mods_t;
...
typedef struct
{
	...
	nxs_string_t					includes;
} nxs_fw_ctl_u_projects_core_modules_distrib_t;
...
typedef struct
{
	...
	nxs_string_t					link_opt;
	nxs_string_t					includes;
	nxs_array_t					distribs;		/* type: nxs_fw_ctl_u_projects_core_modules_distrib_t */
	nxs_array_t					deps;			/* type: nxs_string_t */
	nxs_array_t					libs;			/* type: nxs_string_t */
} nxs_fw_ctl_u_projects_core_modules_el_t;
...
static nxs_cfg_json_state_t nxs_fw_ctl_u_projects_core_modules_read_arr(nxs_process_t *     proc,
                                                                        ...
                                                                        nxs_array_t *       cfg_arr);
static nxs_cfg_json_state_t nxs_fw_ctl_u_projects_core_modules_read_distribs(nxs_process_t *     proc,
                                                                             nxs_json_t *        json,
                                                                             nxs_cfg_json_par_t *cfg_json_par_el,
                                                                             nxs_array_t *       cfg_arr);
...
static nxs_string_t _s_par_distribs			= nxs_string("distribs");
...
nxs_fw_ctl_u_projects_core_modules_t *nxs_fw_ctl_u_projects_core_modules_init(void)
{
	...
	nxs_array_init2(&u_ctx->modules, nxs_fw_ctl_u_projects_core_modules_el_t);
	nxs_array_init2(&u_ctx->selected_modules, nxs_string_t);
	nxs_array_init2(&u_ctx->using.mods, nxs_string_t);
	nxs_array_init2(&u_ctx->using.distribs, nxs_fw_ctl_u_projects_core_modules_using_distrib_t);
...
nxs_fw_ctl_u_projects_core_modules_t *nxs_fw_ctl_u_projects_core_modules_free(nxs_fw_ctl_u_projects_core_modules_t *u_ctx)
{
	...
	nxs_fw_ctl_u_projects_core_modules_distrib_t *d;
	nxs_string_t *                                s;
	size_t                                        i, j;
	...
	for(i = 0; i < nxs_array_count(&u_ctx->modules); i++) {
		...
		for(j = 0; j < nxs_array_count(&el->libs); j++) {
			...
			s = nxs_array_get(&el->libs, j);

			nxs_string_free(s);
		}

		for(j = 0; j < nxs_array_count(&el->distribs); j++) {

			d = nxs_array_get(&el->distribs, j);

			nxs_string_free(&d->name);
			nxs_string_free(&d->link_opt);
			nxs_string_free(&d->includes);
		}

		nxs_array_free(&el->deps);
		nxs_array_free(&el->libs);
		nxs_array_free(&el->distribs);
	}
...
/*
 * Получить строки с инклудами и опциями для линковки для дистрибутива из массива
 */
nxs_fw_ctl_err_t nxs_fw_ctl_u_projects_core_modules_using_get_distrib(nxs_fw_ctl_u_projects_core_modules_t *u_ctx,
                                                                      size_t                                i,
                                                                      nxs_string_t *                        name,
                                                                      nxs_string_t *                        link_opts,
                                                                      nxs_string_t *                        includes)
{
	nxs_fw_ctl_u_projects_core_modules_using_distrib_t *d_u;

	if(u_ctx == NULL || name == NULL || link_opts == NULL || includes == NULL) {

		return NXS_FW_CTL_E_PTR;
	}

	if((d_u = nxs_array_get(&u_ctx->using.distribs, i)) == NULL) {

		return NXS_FW_CTL_E_EXIST;
	}

	nxs_string_clone(name, &d_u->name);
	nxs_string_clone(link_opts, &d_u->link_opts);
	nxs_string_clone(includes, &d_u->includes);
...
void nxs_fw_ctl_u_projects_core_modules_select(nxs_fw_ctl_u_projects_core_modules_t *u_ctx, nxs_array_t *selected_mods)
{
	...
	nxs_array_init2(&select_mods, nxs_fw_ctl_u_projects_core_modules_select_mods_t);
 ...
 * включить.
 * Также, будет заполнена структура "using" с учётом зависимостей в модулях.
 */
...
nxs_fw_ctl_err_t nxs_fw_ctl_u_projects_core_modules_read(nxs_fw_ctl_u_projects_core_modules_t *u_ctx, nxs_string_t *nxs_core_desc_path)
{
	...
	nxs_cfg_json_conf_array_add(&cfg_arr,	&_s_par_nxs_core_mods,	u_ctx,	NULL,	&nxs_fw_ctl_u_projects_core_modules_read_arr,	NXS_CFG_JSON_TYPE_ARRAY_OBJECT,	0,	0,	NXS_YES,	NULL);
                                                                        ...
                                                                        nxs_array_t *       cfg_arr)
{
	...
	nxs_array_init2(&mod->deps, nxs_string_t);
	nxs_array_init2(&mod->libs, nxs_string_t);
	nxs_array_init2(&mod->distribs, nxs_fw_ctl_u_projects_core_modules_distrib_t);
	...
	nxs_cfg_json_conf_array_add(cfg_arr,	&_s_par_distribs,	&mod->distribs,		NULL,	&nxs_fw_ctl_u_projects_core_modules_read_distribs,	NXS_CFG_JSON_TYPE_ARRAY_OBJECT,	0,	0,	NXS_NO,		NULL);
...
static nxs_cfg_json_state_t nxs_fw_ctl_u_projects_core_modules_read_distribs(nxs_process_t *     proc,
                                                                             ...
                                                                             nxs_array_t *       cfg_arr)
{
	nxs_array_t *                                 distribs = nxs_cfg_json_get_val(cfg_json_par_el);
	nxs_fw_ctl_u_projects_core_modules_distrib_t *d;

	d = nxs_array_add(distribs);

	nxs_string_init(&d->name);
	nxs_string_init(&d->link_opt);
	nxs_string_init(&d->includes);

	// clang-format off

	nxs_cfg_json_conf_array_add(cfg_arr,	&_s_par_name,		&d->name,	NULL,	NULL,	NXS_CFG_JSON_TYPE_STRING,	0,	0,	NXS_YES,	NULL);
	nxs_cfg_json_conf_array_add(cfg_arr,	&_s_par_link_opt,	&d->link_opt,	NULL,	NULL,	NXS_CFG_JSON_TYPE_STRING,	0,	0,	NXS_YES,	NULL);
	nxs_cfg_json_conf_array_add(cfg_arr,	&_s_par_includes,	&d->includes,	NULL,	NULL,	NXS_CFG_JSON_TYPE_STRING,	0,	0,	NXS_YES,	NULL);
...
static void nxs_fw_ctl_u_projects_core_modules_using_add_mod(nxs_fw_ctl_u_projects_core_modules_t *u_ctx, nxs_string_t *mod_name)
{
	...
	for(i = 0; i < nxs_array_count(&u_ctx->modules); i++) {
		...
		if(nxs_string_cmp(mod_name, 0, &mod->name, 0) == NXS_YES) {
			...
			for(j = 0; j < nxs_array_count(&mod->distribs); j++) {

				d = nxs_array_get(&mod->distribs, j);

				for(f = NXS_NO, k = 0; k < nxs_array_count(&u_ctx->using.distribs); k++) {

					d_u = nxs_array_get(&u_ctx->using.distribs, k);

					if(nxs_string_cmp(&d->name, 0, &d_u->name, 0) == NXS_YES) {

						nxs_string_printf2_cat(&d_u->link_opts, "%r ", &d->link_opt);
						nxs_string_printf2_cat(&d_u->includes, "%r ", &d->includes);
				...
				if(f == NXS_NO) {
					...
					d_u = nxs_array_add(&u_ctx->using.distribs);

					nxs_string_init3(&d_u->name, &d->name);

					nxs_string_init_empty(&d_u->link_opts);
					nxs_string_init_empty(&d_u->includes);

					nxs_string_printf(&d_u->link_opts, "%r ", &d->link_opt);
					nxs_string_printf(&d_u->includes, "%r ", &d->includes);
				}
			}
...
static void nxs_fw_ctl_u_projects_core_modules_using_mod_free(nxs_fw_ctl_u_projects_core_modules_t *u_ctx)
{
	...
	for(i = 0; i < nxs_array_count(&u_ctx->using.distribs); i++) {

		d_u = nxs_array_get(&u_ctx->using.distribs, i);

		nxs_string_free(&d_u->name);
		nxs_string_free(&d_u->link_opts);
		nxs_string_free(&d_u->includes);
	}

	nxs_array_free(&u_ctx->selected_modules);
	nxs_array_free(&u_ctx->using.mods);
	nxs_array_free(&u_ctx->using.distribs);
...
static void nxs_fw_ctl_u_projects_core_modules_using_mod_cleanup(nxs_fw_ctl_u_projects_core_modules_t *u_ctx)
{
	...
	nxs_array_clear(&u_ctx->selected_modules);
	nxs_array_clear(&u_ctx->using.mods);
	nxs_array_clear(&u_ctx->using.distribs);
