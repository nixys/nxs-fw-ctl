...
static nxs_fw_ctl_err_t nxs_fw_ctl_u_procs_make_makefiles(nxs_fw_ctl_u_procs_t *u_ctx);
static nxs_fw_ctl_err_t nxs_fw_ctl_u_procs_make_genfiles(nxs_fw_ctl_u_procs_t *u_ctx);
...
nxs_fw_ctl_err_t nxs_fw_ctl_u_procs_add(nxs_fw_ctl_u_procs_t *u_ctx)
{
	...
	if(nxs_fw_ctl_u_procs_make_makefiles(u_ctx) != NXS_FW_CTL_E_OK) {

		nxs_error(rc, NXS_FW_CTL_E_ERR, error);
	}

	if(nxs_fw_ctl_u_procs_make_genfiles(u_ctx) != NXS_FW_CTL_E_OK) {
...
static nxs_fw_ctl_err_t nxs_fw_ctl_u_procs_make_makefiles(nxs_fw_ctl_u_procs_t *u_ctx)
{
	...
	for(i = 0; nxs_string_len(&makefiles[i].src) > 0; i++) {
		...
		nxs_string_printf_dyn(&tpl_path,
		                      "%r/" NXS_FW_CTL_DIR_ADD_TPL "/" NXS_FW_CTL_DIR_PROCS "/%r",
