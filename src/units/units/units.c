...
static nxs_fw_ctl_err_t nxs_fw_ctl_u_units_make_makefiles(nxs_fw_ctl_u_units_t *u_ctx);
static nxs_fw_ctl_err_t nxs_fw_ctl_u_units_make_genfiles(nxs_fw_ctl_u_units_t *u_ctx);
...
{
	{nxs_string("subunits.h_tpl"),		nxs_string("" NXS_FW_CTL_U_UNITS_TPL_U_FILENAME "-subunits.h")},
	{nxs_string("typedefs.h_tpl"),		nxs_string("" NXS_FW_CTL_U_UNITS_TPL_U_FILENAME "-typedefs.h")},
	{nxs_string("unit.c_tpl"),		nxs_string("" NXS_FW_CTL_U_UNITS_TPL_U_FILENAME ".c")},
	{nxs_string("unit.h_tpl"),		nxs_string("" NXS_FW_CTL_U_UNITS_TPL_U_FILENAME ".h")},
...
nxs_fw_ctl_u_units_t *nxs_fw_ctl_u_units_init(void)
{
	...
	nxs_array_init(&u_ctx->units_chain, 0, sizeof(nxs_string_t), 1);
        ...
        nxs_fw_ctl_u_units_setup(nxs_fw_ctl_u_units_t *u_ctx, nxs_string_t *proj_name, nxs_string_t *proj_root, nxs_string_t *u_name_str)
{
	...
	for(o = 0, b = nxs_string_str(u_name_str), l = nxs_string_len(u_name_str);
		...
		nxs_string_ncpy_dyn(s, 0, u_name_str, o, c - b - o);
	}
	...
	if(o < l) {
		...
		nxs_string_ncpy_dyn(s, 0, u_name_str, o, l - o);
	}
	...
	nxs_string_printf_dyn(&u_ctx->ppath, "%r/src/units/%r", proj_root, &tmp_str);
	...
	nxs_string_printf_dyn(&u_full_name, "%r-u", proj_name);

	for(i = 0; i < l; i++) {

		s = nxs_array_get(&u_ctx->units_chain, i);

		nxs_string_printf2_cat_dyn(&u_full_name, "-%r", s);
	}
...
nxs_fw_ctl_err_t nxs_fw_ctl_u_units_add(nxs_fw_ctl_u_units_t *u_ctx)
{
	...
	if(nxs_fw_ctl_u_units_make_makefiles(u_ctx) != NXS_FW_CTL_E_OK) {

		nxs_error(rc, NXS_FW_CTL_E_ERR, error);
	}

	if(nxs_fw_ctl_u_units_make_genfiles(u_ctx) != NXS_FW_CTL_E_OK) {
...
nxs_fw_ctl_err_t nxs_fw_ctl_u_units_del(nxs_fw_ctl_u_units_t *u_ctx)
{
	...
	for(i = 0; i < nxs_array_count(&u_ctx->units_chain); i++) {
		...
		if(i > 0) {
			...
			nxs_string_char_add_char_dyn(&unit_name, (u_char)'.');
		}

		s = nxs_array_get(&u_ctx->units_chain, i);

		nxs_string_printf2_cat_dyn(&unit_name, "%r", s);
	}
	...
	nxs_log_write_console(&process, "\tnext units will be deleted from project '%s':", nxs_string_str(&u_ctx->proj_name));
	nxs_log_write_console(&process, "\t* %s", nxs_string_str(&unit_name));

	for(i = 0; i < nxs_array_count(&sub_objects); i++) {

		s = nxs_array_get(&sub_objects, i);

		nxs_log_write_console(&process, "\t* %s.%s", nxs_string_str(&unit_name), nxs_string_str(s));
	}
...
static nxs_fw_ctl_err_t nxs_fw_ctl_u_units_make_fs_struct(nxs_fw_ctl_u_units_t *u_ctx)
{
	...
	if(nxs_fs_mkdir(&u_ctx->path, NXS_FW_CTL_DIR_MODE_DEF) < 0) {
		...
		if(errno == EEXIST) {
			...
			nxs_log_write_error(&process, "can't create new unit: unit exist (unit path: %s)", nxs_string_str(&u_ctx->path));
			...
			nxs_log_write_error(&process,
			                    "can't create unit fs structure: %s (creating dir: %s)",
			                    strerror(errno),
...
static nxs_fw_ctl_err_t nxs_fw_ctl_u_units_make_makefiles(nxs_fw_ctl_u_units_t *u_ctx)
{
	...
	for(i = 0; nxs_string_len(&makefiles[i].src) > 0; i++) {
		...
		nxs_string_printf_dyn(&tpl_path,
		                      "%r/" NXS_FW_CTL_DIR_ADD_TPL "/" NXS_FW_CTL_DIR_UNITS "/%r",
...
static nxs_fw_ctl_err_t nxs_fw_ctl_u_units_make_sub_headers(nxs_fw_ctl_u_units_t *u_ctx)
{
	...
	for(i = 0; i < nxs_array_count(&sub_els); i++) {
		...
		nxs_string_printf2_cat_dyn(&unit_headers, "#include <units/%r%r/%r.h>\n", &u_ctx->prpath, s, s);
		nxs_string_printf2_cat_dyn(&unit_typedefs, "#include <units/%r%r/%r-typedefs.h>\n", &u_ctx->prpath, s, s);
	}

	nxs_string_char_cat_dyn(&unit_headers, (u_char *)NXS_FW_CTL_U_UNITS_TPL_U_HEADERS_E);
	nxs_string_char_cat_dyn(&unit_typedefs, (u_char *)NXS_FW_CTL_U_UNITS_TPL_U_HEADERS_TYPEDEF_E);
	...
	if(u_ctx->level == 0) {
		...
		nxs_string_printf_dyn(&header_path, "%rsrc/%r-units.h", &u_ctx->proj_root, &u_ctx->proj_name);
	}
	else {
		...
		nxs_string_printf_dyn(&header_path, "%r%r-subunits.h", &u_ctx->ppath, s);
	}
